...
export default function SnapPolishApp() {
  const [settings, setSettings] = useState({
    ...
    aspectRatio: 'auto',
    windowControls: true, // ← you already toggle this, so define it here
  });
  ...
  const handleUpload = (e, targetState) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!result) return;
      if (targetState === 'main') setImage(result);
      if (targetState === 'bg')
        setSettings((s) => ({
          ...s,
      ...
      if (targetState === 'logo')
        setSettings((s) => ({ ...s, watermarkLogo: result }));
    };
  ...
  const getBackgroundStyle = () => {
    if (settings.bgType === 'image' && settings.customBackground) {
      return {
        background: `url(${settings.customBackground}) center/cover`,
      };
    }
    ...
    if (settings.pattern === 'dots') {
      return {
        backgroundColor: baseBg.includes('gradient')
          ? '#f8fafc'
          : baseBg,
        backgroundImage:
  ...
  const handleDownload = async () => {
    ...
    try {
      if (typeof window.html2canvas === 'undefined') {
        alert(
          'İndirme aracı yükleniyor, lütfen 3 saniye sonra tekrar deneyin.'
        );
    ...
    <div className="h-screen w-full bg-slate-950 text-slate-200 font-sans flex flex-col-reverse md:flex-row overflow-hidden">
      ...
      <div
        className={`bg-slate-900 border-t md:border-t-0 md:border-r border-slate-800 flex flex-col z-30 shadow-2xl transition-all duration-300 ease-in-out ${
          showSidebar
            ? 'h-[55vh] md:h-screen w-full md:w-80'
            : 'h-12 md:h-screen w-full md:w-0 overflow-hidden opacity-0'
        }`}
      >
        <div className="p-4 border-b border-slate-800 flex items-center justify-between gap-2 min-w-[320px]">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-bold text-base md:text-lg">
              SnapPolish{' '}
              <span className="text-xs font-normal text-cyan-400 bg-cyan-950 px-1.5 py-0.5 rounded ml-1">
                v3.1 (Ultra HD)
              </span>
            </h1>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar min-w-[320px]">
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
              <section className="space-y-3">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                  <Aperture size={14} /> Presets
                </label>
                <div className="grid grid-cols-3 gap-2">
                  <button
                    onClick={() => applyPreset('minimal')}
                    className="p-2 bg-slate-800 border border-slate-700 rounded-lg text-xs hover:bg-slate-700 transition"
                  >
                    Minimal
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="p-2 bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/50 text-amber-500 rounded-lg text-xs hover:bg-amber-500/30 transition"
                  >
                    Pro
                  </button>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['browser', 'phone', 'none'].map((m) => (
                    <button
                      key={m}
                      onClick={() =>
                        setSettings((prev) => ({ ...prev, mockup: m }))
                      }
                      className={`p-2 rounded-lg text-xs border capitalize ${
                        settings.mockup === m
              ...
              <section className="space-y-4">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                  <LayoutTemplate size={14} /> Size & Padding
                </label>
                <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                  {aspectOptions.map((opt) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-md text-xs whitespace-nowrap border ${
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full h-1.5 bg-slate-800 rounded-lg accent-cyan-500"
                  />
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-slate-400">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((prev) => ({
                          ...prev,
                          tilt: parseInt(e.target.value, 10),
                        }))
                      }
                      className="w-full h-1.5 bg-slate-800 rounded-lg accent-cyan-500"
                    />
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <section className="space-y-3">
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    ...
                    className="flex-1 text-[10px] py-1 border border-slate-700 rounded bg-slate-800 text-slate-400 hover:text-white"
                  >
                    Dots
                  </button>
                  <button
                  ...
                  <button
                    onClick={() =>
                      setSettings((prev) => ({ ...prev, pattern: 'none' }))
                    }
                    className="flex-1 text-[10px] py-1 border border-slate-700 rounded bg-slate-800 text-slate-400 hover:text-white"
                  >
                    Solid
                ...
                <div className="space-y-3 p-3 bg-slate-800/50 rounded-xl border border-slate-800">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full h-1.5 bg-slate-700 rounded-lg accent-amber-400"
                    />
                  </div>
                  <div>
                    <div className="flex justify-between text-[10px] text-slate-400 mb-1">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`p-2 rounded border text-xs flex items-center justify-center gap-2 ${
                    settings.darkMode
                      ? 'bg-slate-700 text-white'
                      : 'bg-slate-800 text-slate-400'
                  }`}
                >
                  <Monitor size={14} /> Dark Mode
                </button>
                <button
                  onClick={() =>
                    setSettings((prev) => ({
                      ...prev,
                      windowControls: !prev.windowControls,
                    }))
                  }
                  className={`p-2 rounded border text-xs flex items-center justify-center gap-2 ${
          ...
          {activeTab === 'branding' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                {settings.showWatermark && (
                  <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-800 space-y-3">
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1.5 text-xs text-slate-200 outline-none"
                      placeholder="Brand Name"
                    />
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => logoInputRef.current?.click()}
                        className="flex-1 bg-slate-700 hover:bg-slate-600 text-xs py-1.5 rounded text-slate-300"
                      >
                        Upload Logo
                      </button>
                      <input
              ...
              <section className="space-y-3">
                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2">
                  <Type size={14} /> Promo Badge{' '}
                  <span className="text-[10px] bg-amber-500/20 text-amber-500 px-1 rounded ml-1">
                    NEW
                  </span>
                </label>
                <div className="bg-slate-800/50 p-3 rounded-xl border border-slate-800 space-y-3">
                  ...
                  <div>
                    <div className="flex justify-between text-[10px] text-slate-400 mb-1">
                      <span>Rotation</span>
                      <span>{settings.badgeRotation}°</span>
                    </div>
                    <input
                      type="range"
                      min="-45"
                      max="45"
                      value={settings.badgeRotation}
                      onChange={(e) =>
                        setSettings((prev) => ({
                          ...prev,
                          badgeRotation: parseInt(e.target.value, 10),
                        }))
                      }
                      className="w-full h-1.5 bg-slate-700 rounded-lg accent-amber-400"
                    />
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-1">
                          {['#ef4444', '#f59e0b', '#10b981', '#3b82f6'].map(
                            (c) => (
                              <button
                                key={c}
                                onClick={() =>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 min-w-[320px] flex gap-2">
          ...
          <button
            ...
            className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-3 rounded-xl border border-slate-700 text-sm font-medium flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="flex-[2] bg-cyan-600 hover:bg-cyan-500 text-white py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isDownloading ? (
      ...
      <div className="flex-1 bg-slate-950 relative overflow-hidden flex items-center justify-center p-4 md:p-8 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:16px_16px]">
        ...
        >
          <div
            className={`relative transition-all duration-300 ${getShadowClass(
              settings.shadow
            )} ${
            ...
            style={{
              ...
              transform: `scale(${settings.scale / 100}) rotate(${
                settings.rotate
              }deg) rotateX(${settings.tilt}deg) rotateY(${
                settings.tilt / 2
              }deg)`,
              transformStyle: 'preserve-3d',
              backgroundColor:
                settings.mockup === 'none'
                  ? 'transparent'
                  : settings.darkMode
                  ? '#0f172a'
                  : '#ffffff',
          ...
          >
            {settings.mockup === 'browser' && (
              <div
                className={`h-6 md:h-8 px-3 flex items-center gap-2 border-b ${
                  settings.darkMode
              ...
              >
                ...
                <div
                  className={`ml-2 flex-1 h-4 rounded flex items-center px-2 opacity-40 text-[8px] md:text-[10px] ${
                    settings.darkMode
                      ? 'bg-slate-800 text-slate-400'
            ...
            <img
              ...
              style={{
                maxHeight:
                  settings.aspectRatio !== 'auto' ? '100%' : '60vh',
                minWidth:
                  settings.mockup === 'phone' ? '280px' : 'auto',
        ...
        <div className="hidden md:flex absolute bottom-6 gap-2 bg-slate-800/90 backdrop-blur border border-slate-700 p-2 rounded-xl shadow-xl z-30">
          <button
            ...
            className={`p-2 text-slate-400 hover:text-white rounded-lg transition ${
              !showSidebar && 'text-cyan-400 bg-slate-700'
            }`}
          >
            <Sliders size={18} />
          </button>
          ...
          >
            <Maximize
              size={18}
              className={isFullscreen ? 'rotate-180 transition' : 'transition'}
            />
          </button>
        </div>
      </div>
